...
import com.chat.authorizers.authentication.MyAuthenticatorProvider;
 ...
 * @since 4.0.0
 */
public class ChatExtensionEntry implements ExtensionMain {

    private static final @NotNull Logger log = LoggerFactory.getLogger(ChatExtensionEntry.class);

    @Override
    public void extensionStart(final @NotNull ExtensionStartInput extensionStartInput, final @NotNull ExtensionStartOutput extensionStartOutput) {

        try {
            addSecurityAuthorizers();
    ...
    private void addSecurityAuthorizers(){
        //Connect Authorizer (login)
        Services.securityRegistry().setAuthenticatorProvider(new MyAuthenticatorProvider());
        //Authorizers from subscriptions or publications (pubsub)
        Services.securityRegistry().setAuthorizerProvider((authorizerProviderInput)-> new AppSubscribePublishAuthorizer());
    }
