...
public interface SessionMapper {
    @Insert("INSERT INTO session (id, user_id, presence) VALUES(#{id}, #{userId}, #{presence}) ON DUPLICATE KEY UPDATE presence = #{presence}")
    void addOrUpdateSession(@Param("id") String id, @Param("userId") String userId, @Param("presence") String presence);

    @Select("SELECT user_id FROM session WHERE id = #{sessionId} ORDER BY last_presence DESC LIMIT 1")
